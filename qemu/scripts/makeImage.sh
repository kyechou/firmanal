...
if [ ! -e "${WORK_DIR}/${IID}.tar.gz" ]; then
    if [ ! -e "${FIRMWARE_DIR}/${IID}.tar.gz" ]; then
        echo "Error: Cannot find tarball of root filesystem for ${IID}!"
        exit 1
    else
        cp "${FIRMWARE_DIR}/${IID}.tar.gz" "${WORK_DIR}/${IID}.tar.gz"
    fi
fi
...
cp "${VM_SCRIPT_DIR}/fixImage.sh" "${IMAGE_DIR}"
...
cp "${VM_SCRIPT_DIR}/preInit.sh" "${IMAGE_DIR}/firmadyne/preInit.sh"
