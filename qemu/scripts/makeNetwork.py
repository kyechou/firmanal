...
def qemuCmd(iid, network, arch, endianness, cate):
    ...
    if cate == 'run':
        return QEMURUNTEMPLATE % {'IID': iid, 'GUESTIP': ip, 'NETDEVIP': closeIp(ip),
...
def process(infile, iid, arch, endianness=None, makeQemuCmd=False, outfile_run=None, outfile_terminate=None):
    ...
    for n in network:
        if n[0] not in ips:
            ips.add(n[0])
            if makeQemuCmd:
                qemuCommandRun = qemuCmd(iid, n, arch, endianness, 'run')
                qemuCommandTerminate = qemuCmd(iid, n, arch, endianness, 'terminate')
